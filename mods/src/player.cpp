...
namespace Player
{
    ...
    Camera_Handler::Camera_Handler(Advanced_Struct* advanced_struct, std::string name, std::string scene_name, Transform_Object* attached_transform, Graphic_Object* attached_graphic, Physic_Object* attached_physic) : Object(advanced_struct, name, scene_name, attached_transform, attached_graphic, attached_physic)
    {
    ...
    void* Camera_Handler::clone(Advanced_Struct* game_struct, std::string name, std::string scene_name, Transform_Object* attached_transform, Graphic_Object* attached_graphic, Physic_Object* attached_physic)
    {
    ...
    void Camera_Handler::late_update()
    {
        update_move();
    }

    // Rotate the plan in theCamera_Handler
    void Camera_Handler::rotate_plan(glm::vec3 a_rotation)
    {
    ...
    // Change the anchored position in the Camera_Handler
    void Camera_Handler::set_anchored_position(glm::vec3 anchored_position)
    {
        get_attached_transform()->set_anchored_position(anchored_position);
        get_attached_transform()->set_position(anchored_position);
    }

    // Update the moving of the camera handler
    void Camera_Handler::update_move()
    {
