...
std::string to_uppercase(std::string str)
{
	...
	{
		char chr = static_cast<char>(str[i]);
		int chr_ascii = chr;
...
namespace scls {
    ...
    void Transform_Object::calculate_direction()
    {
        ...
        for (int i = 0; i < static_cast<int>(get_children()->size()); i++)
        {
    ...
    _Window_Base_Struct::_Window_Base_Struct(bool& cursor_on_window, double& mouse_x, double& mouse_y, int& window_width, int& window_height, std::string exec_path): a_exec_path(exec_path), a_cursor_on_window(cursor_on_window), a_last_mouse_x(mouse_x), a_last_mouse_y(mouse_y), a_mouse_x(mouse_x), a_mouse_y(mouse_y), a_window_height(window_height), a_window_width(window_width){}

    // _Window__Window_Base_Struct destructor
    _Window_Base_Struct::~_Window_Base_Struct(){}
    ...
    std::string _Window_Base_Struct::file_formatted(std::string path) {
        ...
        for (int i = 1; i < static_cast<int>(path.size()); i++) // Count the . in the path
        {
            if (static_cast<char>(path[i]) == '.')
            {
                point_count++;
